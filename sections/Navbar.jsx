...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-70 max-w-sm bg-orange-100/95 z-40 lg:hidden transform transition-transform duration-300 ease-in-out border-l border-amber-600/50 shadow-2xl ${
            ...
            {navLinks.map(({ name, href }) => (
              <li key={name}>
                <Link
                  href={href}
                  onClick={closeMobileMenu}
                  className="block text-lg font-semibold text-gray-700 hover:text-amber-600 transition-colors duration-300 py-2 border-b border-gray-100 hover:border-amber-200"
                >
                  {name}
                </Link>
              </li>
